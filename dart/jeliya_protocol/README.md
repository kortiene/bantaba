...
Transport-agnostic Dart client for the Jeliya protocol
...
and the **mobile in-process FFI** transport (the engine linked into the app —
no daemon, no socket), behind one `Client` interface. This is the concrete
realization of the July 2026 native-evaluation architecture decision.
...
| `lib/src/methods.dart` | `extension JeliyaMethods on Client` — one typed wrapper per PROTOCOL.md RPC (24) plus typed push streams `roomEvents` and `peersChanged`. |
...
| `lib/src/frame_router.dart` | The shared frame-routing seam both transports consume (push fan-out with non-Map degradation, id-correlated completion, malformed frames dropped) — the envelope rules cannot drift between WebSocket and FFI. |
| `lib/src/ffi_client.dart` | In-process FFI transport: `FfiClient` drives the Rust engine linked into the app over one Dart `NativePort` (replies and pushes as UTF-8 JSON frames). Honest lifecycle — never `reconnecting` (no transport exists that can drop independently of the process); `stop()` throws on an unclean engine teardown. Exported via `package:jeliya_protocol/ffi.dart` only (keeps `dart:ffi`/`dart:isolate` out of the main barrel; the package stays dependency-free). |
...
- `test/ffi_conformance_test.dart` replays the same corpus against `FfiClient`
  over a host-built `libjeliya_ffi` dylib (`cargo build -p jeliya-ffi`) — the
  **third oracle**, run in CI on every PR — plus the `FfiClient` lifecycle
  contract (stop/start, engine death observed, error-triple fidelity across the
  port).
...
- `test/review_regressions_test.dart` pins review-confirmed fixes so they
...
The Flutter app that consumes this package — the three-pane desktop shell and
the mobile bottom-tab shell — lives in [`../../app`](../../app).
